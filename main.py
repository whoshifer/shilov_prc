...
async def start(message: types.Message):
    ...
    logger.info(f"Пользователь {message.from_user.username} (id={user_id}) запустил бота.")
    await message.reply('Здравствуйте, Вас приветствует чат-бот ПРК.', reply_markup=markup)
...
async def student_command(message: types.Message):
    stud_buttons = ["Заказать справку", "Расписание", "График консультаций", "Помощь", "Автопостинг из группы ВК", "Графики уч.процесса", "Полезные материалы для ВКР", "Полезные материалы для реферата"]
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add(*stud_buttons)
    await message.reply('Выберите то, что вам необходимо. \nДля ознакомления с командами, нажмите кнопку Помощь.' , reply_markup=markup)
...
async def consultation(message: types.Message):
    with open('D:\\Diplom\\main\\res\\graphic.jpg', 'rb') as photo:
        await bot.send_photo(message.chat.id, photo)

@dp.message_handler(filters.Regexp('^Графики уч.процесса'))
async def learn_process(message: types.Message):
    files = [
        'D:\\diplom\\main\\res\\Уч. Процесс\\Уч. процесс (очное).pdf',
        'D:\\diplom\\main\\res\\Уч. Процесс\\Уч.процесс (очно-заочное).pdf',
        'D:\\diplom\\main\\res\\Уч. Процесс\\Уч.процесс (заочное).pdf'
    ]  # список файлов

    media = [types.InputMediaDocument(types.InputFile(file)) for file in files]  # создать список медиафайлов

    await bot.send_media_group(message.chat.id, media=media)  # отправить группу файлов


@dp.message_handler(filters.Regexp('^Полезные материалы для ВКР'))
async def vkr(message: types.Message):
    files = [
        'D:\\diplom\\main\\res\\ВКР\\Методические указания по ВКР.pdf',
        'D:\\diplom\\main\\res\\ВКР\\Нормоконтроль 2023.pdf', 
        'D:\\diplom\\main\\res\\ВКР\\Требования к презентации ВКР.pdf',
        'D:\\diplom\\main\\res\\ВКР\\Рамка для дипломного проекта.pdf',
        'D:\\diplom\\main\\res\\ВКР\\Рамка для дипломной работы.pdf'
    ]
...
async def referat(message: types.Message):
    files = [
        'D:\\diplom\\main\\res\\Реферат\\Методические рекомендации по выполнению реферативных работ.pdf',
        'D:\\diplom\\main\\res\\Реферат\\Нормоконтроль. Новый Реферат.pdf', 
        'D:\\diplom\\main\\res\\Реферат\\Пример оформления реферата. Новый.pdf'
    ]
    media = [types.InputMediaDocument(types.InputFile(file)) for file in files]
