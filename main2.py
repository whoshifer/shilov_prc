...
async def handle_feedback_message(message: types.Message):
  ...
  if feedback_enabled:
    logger.info(
      f'Пользователь {message.from_user.username} (id={message.from_user.id}) оставил обратную связь : "{message.text}"'
    )
    feedback_enabled = False
...
async def check_new():
  ...
  while True:
    ...
    post_short_link = vk_session.method('utils.getShortLink',
                                        {'url': post_link})['short_url']
    ...
    for user_id in users:
      if autoposting_statuses.get(user_id, True):
        if len(media_group
               ) > 0:  # добавить проверку на наличие элементов в media_group
...
async def vk_parse():
  ...
  if not os.path.exists("files"):
    os.makedirs("files")
  while True:
    response = vk.board.getComments(group_id=group_id,
                                    topic_id=topic_id,
                                    count=100)
    new_comments = [
      comment for comment in response["items"]
    ...
    if new_comments:
      ...
      if "attachments" in last_comment:
        ...
        doc_attachments = [
          ...
          if a["type"] == "doc" and a["doc"]["ext"] == "xlsx"
        ]
        if doc_attachments:
